 ...
 * This rule accesses the app's SettingsRepository and sets the flag to true,
 ...
 * This rule accesses the app's SettingsRepository and sets the app lock flag to false,
    ...
    override fun apply(
        ...
        return object : Statement() {
            override fun evaluate() {
                ...
                // Clear any stored budgets from DataStore to avoid cross-test contamination
                runBlocking {
                    context.financeSettingsDataStore.edit { prefs ->
                        val keysToRemove = prefs.asMap().keys.filter { it.name.startsWith("overall_budget_") }
                ...
                runBlocking {
                    ...
                    try {
                        ...
                        writableDb.execSQL("DELETE FROM custom_sms_rules")
                        writableDb.execSQL("DELETE FROM merchant_rename_rules")
                        writableDb.execSQL("DELETE FROM merchant_category_mapping")
                        writableDb.execSQL("DELETE FROM merchant_mappings")
                        writableDb.execSQL("DELETE FROM sms_parse_templates")
                        writableDb.execSQL("DELETE FROM account_aliases")
                        writableDb.execSQL("DELETE FROM deleted_sms_hashes")
                        writableDb.execSQL("DELETE FROM merge_records")
                        writableDb.execSQL("DELETE FROM recurring_patterns")
                        writableDb.execSQL("DELETE FROM goal_transaction_links")
                        writableDb.execSQL("DELETE FROM goal_contributions")
