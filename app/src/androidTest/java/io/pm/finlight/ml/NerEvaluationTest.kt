...
class NerEvaluationTest {
    ...
    private val testCases = listOf(
        // HDFC
        TestCase("HDFC", "AM-HDFCBK",
            "Sent Rs.11.00\nFrom HDFC Bank A/C *1243\nTo Raju\nOn 07/08/25\nRef 558523453508",
            11.0, "Raju"),
        TestCase("HDFC", "JD-HDFCBK",
        ...
        TestCase("HDFC", "VM-HDFCBK",
            ...
            4000.0, "Quant Mutual Fund"),

        // ICICI
        TestCase("ICICI", "DM-ICIBNK",
    ...
    fun runNerEvaluationAndWriteReport() {
        ...
        runBlocking {
            for ((idx, tc) in testCases.withIndex()) {
                ...
                // Run NER extraction
                val nerEntities = try { nerExtractor.extract(tc.smsBody) } catch (e: Exception) { emptyMap() }

                // Baseline (no NER)
                val baselineResult = SmsParser.parseWithReason(
                    sms = smsMessage,
                ...
                // NER-enabled
                val nerResult = SmsParser.parseWithReason(
                    sms = smsMessage,
                    mappings = emptyMappings,
                    customSmsRuleProvider = customSmsRuleProvider,
                    merchantRenameRuleProvider = merchantRenameRuleProvider,
        ...
        // Write CSV report
        val csv = buildString {
            appendLine("Category,SMS_Snippet,Verdict,Expected_Amount,Baseline_Amount,NER_Amount,Expected_Merchant,Baseline_Merchant,NER_Merchant,NER_Raw")
            for (r in results) {
                appendLine(listOf(
                    r.category,
                    "\"${r.smsSnippet}\"",
