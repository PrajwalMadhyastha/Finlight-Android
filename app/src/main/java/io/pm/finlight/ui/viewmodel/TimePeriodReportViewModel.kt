...
class TimePeriodReportViewModel(
    ...
    val excludedIncomeMonths: StateFlow<Set<String>> =
        settingsRepository.getExcludedIncomeMonths()
            .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptySet())
    val excludedExpenseMonths: StateFlow<Set<String>> =
        settingsRepository.getExcludedExpenseMonths()
            .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptySet())

    val yearlyMonthlyBreakdown: StateFlow<List<MonthlyBreakdown>> =
        combine(
    ...
    val totalIncome: StateFlow<Long> =
        combine(transactionsForPeriod, yearlyMonthlyBreakdown) { transactions, breakdowns ->
    ...
    val monthlyAverageIncome: StateFlow<Double?> =
        combine(yearlyMonthlyBreakdown, _selectedDate) { breakdowns, date ->
    ...
    val monthlyAverageExpenses: StateFlow<Double?> =
        combine(yearlyMonthlyBreakdown, _selectedDate) { breakdowns, date ->
            if (timePeriod != TimePeriod.YEARLY) return@combine null
            val elapsedMonths = getElapsedMonthsInYear(date)
