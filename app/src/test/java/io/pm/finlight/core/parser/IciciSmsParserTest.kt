...
class IciciSmsParserTest : BaseSmsParserTest() {
    @Test
    fun `test parses ICICI debit with ACH and Avl Bal`() =
        runBlocking {
            setupTest()
    ...
    @Test
    fun `test parses ICICI debit message with Rs symbol correctly`() =
        runBlocking {
            setupTest()
    ...
    @Test
    fun `test parses ICICI ACH credit and account`() =
        runBlocking {
            setupTest()
    ...
    @Test
    fun `test parses ICICI UPI reversal and account`() =
        runBlocking {
            setupTest()
    ...
    @Test
    fun `test parses ICICI NEFT debit and account`() =
        runBlocking {
            setupTest()
            ...
            assertEquals("expense", result?.transactionType)
            assertEquals("NEFT*0005", result?.merchantName)
            assertNotNull("Account info should be parsed", result?.potentialAccount)
            assertEquals("ICICI Bank Account XX508", result?.potentialAccount?.formattedName)
            assertEquals("Bank Account", result?.potentialAccount?.accountType)
        }
    ...
    @Test
    fun `test parses ICICI Card spend and account`() =
        runBlocking {
            setupTest()
    ...
    @Test
    fun `test parses ICICI Card spend with 'spent using' format`() =
        runBlocking {
            setupTest()
    ...
    @Test
    fun `test parses ICICI AutoPay debit`() =
        runBlocking {
            setupTest()
    ...
    @Test
    fun `test parses ICICI IMPS credit with mobile number`() =
        runBlocking {
            setupTest()
    ...
    @Test
    fun `test parses ICICI credit card refund without 'your'`() =
        runBlocking {
            setupTest()
    ...
    @Test
    fun `test parses ICICI debit with account at start and bank at end`() =
        runBlocking {
            setupTest()
            ...
            assertEquals("expense", result?.transactionType)
            assertEquals("linked user635", result?.merchantName)
            assertNotNull(result?.potentialAccount)
    ...
    @Test
    fun `test parses ICICI credit with 'We have credited'`() =
        runBlocking {
            setupTest()
    ...
    @Test
    fun `test parses debit with recipient credited as expense`() =
        runBlocking {
            setupTest()
