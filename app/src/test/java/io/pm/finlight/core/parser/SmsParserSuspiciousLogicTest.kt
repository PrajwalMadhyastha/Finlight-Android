...
class SmsParserSuspiciousLogicTest : BaseSmsParserTest() {
    ...
    @Test
    fun `Option A - amount exceeding 100,000 flags transaction for review`() = runBlocking {
        setupTest()
        val sms = SmsMessage(
            id = 1,
            sender = "AM-HDFCBK",
    ...
    @Test
    fun `Option C - amount exceeding available balance flags transaction for review`() = runBlocking {
        setupTest()
        val sms = SmsMessage(
            id = 2,
            sender = "DM-ICIBNK",
        ...
        val nerEntities = mapOf(
            ...
            "BALANCE" to NerEntity("100.00", 0.95f) // Balance is less than Amount
        )
    ...
    @Test
    fun `Option D - NER confidence below threshold flags transaction for review`() = runBlocking {
        setupTest()
        val sms = SmsMessage(
            id = 3,
            sender = "AM-AXIS",
        ...
        val nerEntities = mapOf(
            "AMOUNT" to NerEntity("450.00", 0.65f), // 0.65 is < 0.70 threshold
    ...
    @Test
    fun `Normal transaction does not flag for review`() = runBlocking {
        setupTest()
        val sms = SmsMessage(
            id = 4,
            sender = "AM-HDFCBK",
        ...
        val nerEntities = mapOf(
            "AMOUNT" to NerEntity("250.00", 0.95f), // Confidence > 0.70, Amount < 100000
