...
class CustomRuleParserTest : BaseSmsParserTest() {
    @Test
    fun `test custom rule transactionType 'income' overrides 'spent' keyword`() =
        runBlocking {
            // Arrange
    ...
    @Test
    fun `test custom rule transactionType 'expense' overrides 'credited' keyword`() =
        runBlocking {
            // Arrange
    ...
    @Test
    fun `test custom rule transactionType 'null' falls back to 'spent' keyword`() =
        runBlocking {
            // Arrange
            val smsBody = "You spent Rs. 100 at Test Merchant."
            val sms = SmsMessage(1, "TEST", smsBody, 1L)
    ...
    @Test
    fun `test custom rule transactionType 'null' falls back to 'credited' keyword`() =
        runBlocking {
            // Arrange
            val smsBody = "You were credited Rs. 500 by Test Merchant."
            val sms = SmsMessage(1, "TEST", smsBody, 1L)
