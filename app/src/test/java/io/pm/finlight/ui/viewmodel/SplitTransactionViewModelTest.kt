...
class SplitTransactionViewModelTest : BaseViewModelTest() {
    @Mock
    ...
    @Test
    fun `init for new split populates state from parent transaction`() =
        runTest {
            // Arrange
            val transactionId = 1
            val parentTxn =
    ...
    @Test
    fun `init for new split with foreign currency uses originalAmount`() =
        runTest {
            // Arrange
            val transactionId = 1
            val parentTxn =
                Transaction(id = transactionId, description = "Parent", amount = 830.0, originalAmount = 10.0, currencyCode = "USD", isSplit = false, date = 0L, accountId = 1, categoryId = 1, notes = "Foreign txn")
            `when`(transactionRepository.getTransactionById(transactionId)).thenReturn(flowOf(parentTxn))
    ...
    @Test
    fun `init for existing split loads previous splits`() =
        runTest {
            // Arrange
            val transactionId = 1
            val parentTxn =
    ...
    @Test
    fun `addSplitItem adds a new item and recalculates remaining amount`() =
        runTest {
            // Arrange
            val transactionId = 1
            val parentTxn =
    ...
    @Test
    fun `removeSplitItem removes an item and recalculates remaining amount`() =
        runTest {
            // Arrange
            val transactionId = 1
            val parentTxn =
    ...
    @Test
    fun `recalculateRemainingAmount works correctly`() =
        runTest {
            // Arrange
            val transactionId = 1
            val parentTxn =
