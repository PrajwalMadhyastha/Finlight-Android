...
class BudgetDaoTest {
    ...
    @Test
    fun `getBudgetsWithSpendingForMonth calculates spending correctly`() =
        runTest {
            // Arrange
    ...
    @Test
    fun `getBudgetsWithSpendingForMonth carries over budget from previous month`() =
        runTest {
    ...
    @Test
    fun `getBudgetsWithSpendingForMonth prefers current month budget over carry-over`() =
        runTest {
            // Arrange
            val lastMonthCal = Calendar.getInstance().apply { add(Calendar.MONTH, -1) }
            val currentCal = Calendar.getInstance()
            ...
            budgetDao.insert(
                Budget(
                    categoryName = "Travel",
                    amount = 20000.0,
                    month = lastMonthCal.get(Calendar.MONTH) + 1,
                    year = lastMonthCal.get(Calendar.YEAR),
                ),
            )
    ...
    @Test
    fun `getBudgetsForMonth returns budgets for specific month only`() =
        runTest {
            // Arrange
            val cal = Calendar.getInstance()
            val year = cal.get(Calendar.YEAR)
    ...
    @Test
    fun `getAllBudgets returns all budgets from all months`() =
        runTest {
            // Arrange
    ...
    @Test
    fun `getById returns correct budget`() =
        runTest {
            // Arrange
            budgetDao.insert(Budget(categoryName = "Food", amount = 1.0, month = 1, year = 2025))
    ...
    @Test
    fun `update modifies budget correctly`() =
        runTest {
            // Arrange
    ...
    @Test
    fun `deleteAll removes all budgets`() =
        runTest {
            // Arrange
            budgetDao.insertAll(
                listOf(
