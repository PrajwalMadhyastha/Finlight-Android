...
class TransactionDaoTest {
    ...
    @Test
    fun `getDailySpendingForDateRange correctly groups by day and sums amounts`() =
        runTest {
            // Arrange
            ...
            transactionDao.getDailySpendingForDateRange(todayStart, todayEnd).test {
                ...
                assertEquals(
                    ...
                    0.01,
                    "Total amount should be the sum of all non-excluded expenses (regular + split)",
                )
    ...
    @Test
    fun `getTransactionsForMerchantInRange is case-insensitive`() =
        runTest {
            // Arrange
    ...
    @Test
    fun `insert and get transaction by id`() =
        runTest {
            // Arrange
            val transaction =
                Transaction(
    ...
    @Test
    fun `getTransactionDetailsForRange returns correctly joined data`() =
        runTest {
            // Arrange
            val transactionTime = System.currentTimeMillis()
            val transaction =
    ...
    @Test
    fun `updateDescription correctly updates description`() =
        runTest {
            val id =
                transactionDao.insert(
                    Transaction(description = "Old", amount = 1.0, date = 1L, accountId = 1, categoryId = 1, notes = null),
                ).toInt()
    ...
    @Test
    fun `updateAmount correctly updates amount`() =
        runTest {
            val id =
                transactionDao.insert(
                    Transaction(description = "Test", amount = 1.0, date = 1L, accountId = 1, categoryId = 1, notes = null),
                ).toInt()
    ...
    @Test
    fun `updateNotes correctly updates notes`() =
        runTest {
            val id =
                transactionDao.insert(
                    Transaction(description = "Test", amount = 1.0, date = 1L, accountId = 1, categoryId = 1, notes = "Old Note"),
                ).toInt()
    ...
    @Test
    fun `updateCategoryId correctly updates categoryId`() =
        runTest {
            val id =
                transactionDao.insert(
                    Transaction(description = "Test", amount = 1.0, date = 1L, accountId = 1, categoryId = 1, notes = null),
    ...
    @Test
    fun `updateAccountId correctly updates accountId`() =
        runTest {
            val id =
                transactionDao.insert(
                    Transaction(description = "Test", amount = 1.0, date = 1L, accountId = 1, categoryId = 1, notes = null),
    ...
    @Test
    fun `updateDate correctly updates date`() =
        runTest {
            val id =
                transactionDao.insert(
                    Transaction(description = "Test", amount = 1.0, date = 1L, accountId = 1, categoryId = 1, notes = null),
    ...
    @Test
    fun `updateExclusionStatus correctly updates isExcluded`() =
        runTest {
            val id =
                transactionDao.insert(
                    Transaction(
    ...
    @Test
    fun `updateTransactionType correctly updates transactionType`() =
        runTest {
            val id =
                transactionDao.insert(
                    Transaction(
    ...
    @Test
    fun `searchTransactions by keyword and accountId`() =
        runTest {
            // Arrange
            transactionDao.insert(
                Transaction(
    ...
    @Test
    fun `searchTransactions by categoryId and transactionType`() =
        runTest {
            // Arrange
            transactionDao.insert(
                Transaction(
    ...
    @Test
    fun `searchTransactions by dateRange and tagId`() =
        runTest {
            // Arrange
            ...
            transactionDao.searchTransactions(
                ...
                null,
                null,
                startDate = yesterday - 1000,
                endDate = yesterday + 1000,
                tagId = tag2.id,
            ).test {
                val results = awaitItem()
                assertEquals(1, results.size)
    ...
    @Test
    fun `searchTransactions by keyword, accountId, and tagId`() =
        runTest {
            // Arrange
            val now = System.currentTimeMillis()
            // Target transaction
            val tx1Id =
                transactionDao.insert(
                    Transaction(
                        id = 1,
                        description = "Work Project Dinner",
                        amount = 1200.0,
                        date = now,
                        accountId = 1,
                        categoryId = 1,
            ...
            // Decoy transactions
            transactionDao.insert(
                Transaction(id = 2, description = "Work Lunch", amount = 300.0, date = now, accountId = 2, categoryId = 1, notes = null),
            ) // Wrong account
            transactionDao.insert(
                Transaction(
                    id = 3,
                    description = "Project Snacks",
                    amount = 150.0,
                    date = now,
                    accountId = 1,
                    categoryId = 1,
    ...
    @Test
    fun `searchTransactions by keyword matches notes`() =
        runTest {
            // Arrange
            val transaction =
                Transaction(
                    ...
                    accountId = 1,
                    categoryId = 1,
                    notes = "Important business lunch",
                )
            transactionDao.insert(transaction)
    ...
    @Test
    fun `searchTransactions by accountId filters correctly`() =
        runTest {
            // Arrange
            transactionDao.insert(
                Transaction(
    ...
    @Test
    fun `searchTransactions by categoryId filters correctly`() =
        runTest {
            // Arrange
            transactionDao.insert(
                Transaction(
    ...
    @Test
    fun `searchTransactions by dateRange filters correctly`() =
        runTest {
            // Arrange
            val now = System.currentTimeMillis()
            ...
            transactionDao.insert(
                Transaction(id = 1, description = "today", amount = 100.0, date = now, accountId = 1, categoryId = 1, notes = null),
            )
            transactionDao.insert(
                Transaction(
                    id = 2,
    ...
    @Test
    fun `searchTransactions by tagId filters correctly`() =
        runTest {
            // Arrange
            transactionDao.insert(
                Transaction(
    ...
    @Test
    fun `getFinancialSummaryForRangeFlow calculates income and expenses correctly`() =
        runTest {
            // Arrange
            val now = System.currentTimeMillis()
    ...
    @Test
    fun `getSpendingByCategoryForMonth aggregates and orders correctly`() =
        runTest {
            // Arrange
            val now = System.currentTimeMillis()
    ...
    @Test
    fun `getAverageDailySpendingForRange calculates average correctly`() =
        runTest {
            // Arrange
    ...
    @Test
    fun `getSpendingAnalysisByMerchant groups case-insensitive merchants correctly`() =
        runTest {
    ...
    @Test
    fun `getSpendingAnalysisByMerchant_respects_includeExcluded_flag`() =
        runTest {
            // Arrange
            val transactionTime = System.currentTimeMillis()
            val startTime = transactionTime - TimeUnit.HOURS.toMillis(1)
    ...
    @Test
    fun `getSpendingAnalysisByCategory_respects_includeExcluded_flag`() =
        runTest {
            // Arrange
            val transactionTime = System.currentTimeMillis()
            val startTime = transactionTime - TimeUnit.HOURS.toMillis(1)
    ...
    @Test
    fun `getSpendingAnalysisByTag_respects_includeExcluded_flag`() =
        runTest {
            // Arrange
            val transactionTime = System.currentTimeMillis()
            val startTime = transactionTime - TimeUnit.HOURS.toMillis(1)
            ...
            val tx2Id =
                transactionDao.insert(
                    Transaction(
                        description = "Excluded Work Lunch",
                        amount = 50.0,
                        date = transactionTime,
                        accountId = 1,
                        categoryId = 1,
    ...
    @Test
    fun `updateTransactionType_modifiesTypeInDatabase`() =
        runTest {
            // Arrange
            val transaction =
                Transaction(
    ...
    @Test
    fun getRecentManualTransactions_filtersAndGroupsCorrectly() =
        runBlocking {
    ...
    @Test
    fun `searchMerchants returns unique predictions based on description category and account`() =
        runTest {
            // Arrange
            val transactionTime = System.currentTimeMillis()
    ...
    @Test
    fun `findSimilarTransactions strictly matches originalDescription and ignores description`() =
        runTest {
            // Arrange
            transactionDao.insert(
                Transaction(
            ...
            assertTrue(similarToRam.any { it.id == 4 }) // matches originalDescription = RAM
            // Should NOT include id=3 because originalDescription is null
